...
export const useGetServerMapGetOnlyResponseTimeHistogramDataV2 = ({
  ...
  const { data, isLoading } = useQuery<GetServerMapOnlyResponseTimeHistogram.Response>({
    queryKey: [END_POINTS.SERVER_MAP_GET_ONLY_RESPONSE_TIME_HISTOGRAM_DATA_V2, queryString],
    queryFn: queryFn(
      `${END_POINTS.SERVER_MAP_GET_ONLY_RESPONSE_TIME_HISTOGRAM_DATA_V2}${queryString}`,
    ),
    enabled: !!queryString,
  });
