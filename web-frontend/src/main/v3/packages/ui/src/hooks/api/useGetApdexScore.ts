...
export type UseGetApdexScoreProps = {
   ...
   *
   * map의 우측 패널은 `useServerMapRequestServiceName`이 정한 값을 항상 넘긴다 — 고른 노드가
   * 있으면 그 노드의 service, 없으면 화면의 service다. servicemap이 다른 service의 application도
   * 함께 그리기 때문에 둘이 갈릴 수 있다. 넘기지 않으면 기존대로 fetch 인터셉터가
   * 경로/전역 선택값으로 결정한다.
   */
  serviceName?: string;
};
